...
const ProfileDrawer: React.FC<ProfileDrawerProps> = ({
  ...
  const title = useMemo(() => {
    return data.name || otherUser?.name || 'No Title';
  }, [data.name, otherUser?.name]);

  const { members } = useActiveList();
  const isActive = otherUser ? members.includes(otherUser.email!) : false;
                      ...
                      <div className="relative mt-6 flex-1 px-4 sm:px-6">
                        <div className="flex flex-col items-center">
                          <div className="mb-2">
                            ...
                            {data.isGroup ? <AvatarGroup users={data.participants} /> : <Avatar user={otherUser} />}
                          </div>
                          <div>
                            {title}
                          </div>
                              ...
                              {data.isGroup && (
                                <div>
                                  ...
                                  <dd
                                    ...
                                    "
                                  >
                                    {data.participants.map((participant) => participant.user.email).join(', ')}
                                  </dd>
                                </div>
                              )}
                              {!data.isGroup && (
