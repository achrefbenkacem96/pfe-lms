...
function validateQuizQuestions(quizQuestions: QuizQuestion[]): ValidationResponse {
...
type QuizBuildNavProps = {
  quizQuestions: QuizQuestion[];
  newQuiz: Quiz;
  setNewQuiz: React.Dispatch<React.SetStateAction<Quiz>>;
};

function QuizBuildNav({ quizQuestions, newQuiz, setNewQuiz }: QuizBuildNavProps) {
  ...
  async function createNewQuiz() {
    try {
      ...
      console.log(id);
      // Update the id property of the newQuiz object
    ...
    if (selectedQuiz) {
      ...
      try {
        const res = await fetch(`http://localhost:3000/api/quizzes?id=${id}`, {
          ...
          body: JSON.stringify({
            updateQuizQuestions: newQuiz.quizQuestions,
            updateQuiz:newQuiz,
      ...
      createNewQuiz();
      router.push('/dashboard/quiz'); // Navigate to main page
    }
  }
  ...
  return (
    <div className="poppins my-12 flex justify-between items-center ">
      <div className="flex gap-2 items-center">
        <Image src="/quiz/quiz-builder-icon.png" alt="" height={50} width={50} />
